...
async function fetchIfMarkdown(candidate) {
    try {
        ...
        if (/^\s*<(!doctype|html)/i.test(text))
            return null;
        if (text.length < MIN_USEFUL_BYTES)
            return null;
        return text;
    }
    catch {
 ...
 */
async function findMarkdownAlternate(url, html) {
    const candidates = [];
    ...
    for (const c of candidates) {
        ...
        if (content)
            return { url: c.href, content, declared: c.declared };
    }
    return null;
}
