 ...
 */
function targetContext(url: string): { targetId: string; urlLength: number } {
    return {
        targetId: createHash('sha256').update(url).digest('hex').slice(0, 12),
        urlLength: url.length,
    };
}

function logEvent(level: LogLevel, event: string, details: Record<string, unknown> = {}): void {
...
function eligibility(): { eligible: boolean; environment: string; apiKey?: string; reason?: string } {
...
function classifyRequestError(error: unknown): string {
    ...
    if (error instanceof TypeError) return 'network-or-fetch-error';
    return 'request-error';
}

async function requestJinaRender(url: string, apiKey: string): Promise<RenderedContent | null> {
        ...
        try {
            ...
            if (response.status === 429 && attempt < MAX_RETRIES) {
                ...
                const retryDelayMs = Math.min(
                    requestedDelayMs ?? DEFAULT_RETRY_AFTER_MS,
                    MAX_RETRY_AFTER_MS,
                );
 ...
 */
export async function renderWithJina(url: string): Promise<RenderedContent | null> {
    const context = targetContext(url);
    ...
    if (!gate.eligible || !gate.apiKey) return null;
