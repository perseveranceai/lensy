...
import { invokeBedrockForJson, getModelId } from '../shared/bedrock-helpers';
 ...
 */
export const detectStructureTool = tool(
    async (input): Promise<string> => {
        ...
        try {
            ...
            await writeSessionArtifact(input.sessionId, 'structure.json', structure);
    ...
    },
    {
        ...
        schema: z.object({
            ...
            selectedModel: z.string().default('claude').describe('The AI model to use')
        })
    }
);
...
interface DocumentStructure {
    ...
    topics: { name: string; pageCount: number; urls: string[] }[];
    confidence: number;
...
- "mixed": combination of above`;
    ...
    try {
        return await invokeBedrockForJson<DocumentStructure>(prompt, {
        ...
        return {
            ...
            confidence: 50,
            contentType: (contentType === 'product-guide' ? 'product-guide' : 'mixed') as any
        };
    }
}
