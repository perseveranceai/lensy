...
const page_summarizer_1 = require("../shared/page-summarizer");
/**
 * Tool 2: Process URL
 ...
 */
function analyzeUrlSlugQuality(url) {
    try {
        const urlObj = new URL(url);
...
async function copyCachedContentToSession(cacheS3Location, sessionId) {
    ...
    const cached = await (0, s3_helpers_1.readFromS3)(bucketName, cacheS3Location);
    if (!cached)
        throw new Error('Cached content body is empty');
    await (0, s3_helpers_1.writeToS3)(bucketName, `sessions/${sessionId}/processed-content.json`, cached);
    console.log(`Copied cached content from ${cacheS3Location} to session: sessions/${sessionId}/processed-content.json`);
}
async function storeSessionMetadata(sessionId, url, selectedModel) {
    await (0, s3_helpers_1.writeSessionArtifact)(sessionId, 'metadata.json', {
        url,
        selectedModel,
        sessionId,
        startTime: Date.now()
...
async function addCacheMetadata(sessionId, wasFromCache) {
    try {
        ...
        await (0, s3_helpers_1.writeSessionArtifact)(sessionId, 'processed-content.json', processedContent);
...
async function storeProcessedContentInCache(url, processedContent, contextualSetting) {
    try {
        ...
        await (0, s3_helpers_1.writeToS3)(bucketName, cacheS3Key, processedContent);
 ...
 */
async function retryWithGet(url, timeout) {
    try {
...
function validateContentReadability(markdown, url) {
...
function isNonDocumentationUrl(url) {
...
// THE TOOL
// ═══════════════════════════════════════════════════════════════
exports.processUrlTool = (0, tools_1.tool)(async (input) => {
    ...
    try {
        ...
        if (cachedContent && !isCacheExpired(cachedContent)) {
            ...
            const cached = await (0, s3_helpers_1.readSessionArtifact)(input.sessionId, 'processed-content.json');
        ...
        // ── Parse & clean ──────────────────────────────────
        const dom = new jsdom_1.JSDOM(htmlContent, { url: input.url });
        ...
        await progress.success(`Content processed: ${Math.round(htmlContent.length / 1024)}KB -> ${Math.round(cleanHtml.length / 1024)}KB (${reductionPercent}% reduction)`, { originalSize: htmlContent.length, cleanedSize: cleanHtml.length, reductionPercent });
        ...
        if (contextEnabled) {
            ...
            const safeDomain = (0, page_summarizer_1.deriveSafeDomain)(input.url);
            let kbPages = [];
            try {
...
Return ONLY a JSON array of indices, e.g. [0, 3, 7, 12]. Return [] if none are relevant.`;
        ...
        const nonDocTypes = ['blog-post', 'landing-page', 'marketing-page', 'non-documentation'];
        ...
        await (0, s3_helpers_1.writeSessionArtifact)(input.sessionId, 'processed-content.json', processedContent);
        ...
        try {
            const safeDomainForKB = (0, page_summarizer_1.deriveSafeDomain)(input.url);
            const currentPageSummary = await (0, page_summarizer_1.summarizeAndParsePage)(input.url, document.title || input.url, markdownContent, undefined);
            await (0, page_summarizer_1.writePageToKB)(safeDomainForKB, currentPageSummary, 'doc-audit');
    ...
    name: 'process_url',
    description: 'Fetches a documentation URL, strips HTML noise, converts to clean Markdown, ' +
    ...
    schema: zod_1.z.object({
        url: zod_1.z.string().describe('The documentation URL to process'),
        sessionId: zod_1.z.string().describe('The analysis session ID'),
        selectedModel: zod_1.z.string().describe('The selected AI model identifier (e.g., "claude", "auto")'),
        contextAnalysisEnabled: zod_1.z.boolean().optional().default(false).describe('Enable context analysis to discover parent/child/sibling pages'),
        maxContextPages: zod_1.z.number().optional().default(5).describe('Maximum number of context pages to discover'),
        cacheEnabled: zod_1.z.boolean().optional().default(true).describe('Enable DynamoDB caching (7-day TTL). Set false to force fresh analysis')
    })
});
