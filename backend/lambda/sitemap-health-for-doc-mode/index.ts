...
interface SitemapHealthForDocModeEvent {
    url: string;
    sessionId: string;
    analysisStartTime: number;
}

interface SitemapHealthForDocModeResult {
    success: boolean;
    sitemapUrl: string | null;
    discoveryMethod: 'robots.txt' | 'direct' | 'fallback' | 'not-found';
    healthResults: any | null;
    error?: string;
...
class ProgressPublisher {
    ...
    async progress(message: string, phase?: string) {
    ...
    private async sendMessage(type: string, message: string, phase?: string, metadata?: any) {
        if (!this.apiGatewayClient) return;
...
export const handler: Handler<SitemapHealthForDocModeEvent, SitemapHealthForDocModeResult> = async (event) => {
    ...
    try {
        ...
        let skippedGzipUrl: string | null = null; // [NEW] Track skipped Gzip URLs
        let lastDiscoveryError: string | null = null; // [NEW] Track discovery errors
        let discoveryMethod: 'robots.txt' | 'direct' | 'fallback' | 'not-found' = 'not-found';
        ...
        if (!sitemapUrl) {
            ...
            const result: SitemapHealthForDocModeResult = {
                success: true,
                sitemapUrl: null,
        ...
        try {
            ...
            const sitemapResultsResponse = await s3Client.send(new GetObjectCommand({
                Bucket: process.env.ANALYSIS_BUCKET,
        ...
        await progress.success(
            `Sitemap health: ${healthResult.healthyUrls}/${healthResult.totalUrls} URLs healthy (${healthResult.healthPercentage}%)`,
            {
                totalUrls: healthResult.totalUrls,
                healthyUrls: healthResult.healthyUrls,
                healthPercentage: healthResult.healthPercentage
