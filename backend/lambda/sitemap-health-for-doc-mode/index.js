...
const handler = async (event) => {
    ...
    try {
        ...
        try {
            const cachedResponse = await s3Client.send(new client_s3_1.GetObjectCommand({
            ...
            if (cachedResponse.Body) {
                ...
                if (ageMs < CACHE_TTL_MS) {
                    ...
                    await progress.success(`Sitemap health (cached ${ageLabel} ago): ${cachedData.healthyUrls}/${cachedData.totalUrls} URLs healthy (${cachedData.healthPercentage}%)`, { cached: true, cacheAge: ageLabel, ...cachedData });
        ...
        const estimateLabel = estimatedSeconds >= 60
            ...
            : `~${estimatedSeconds}s`;
        await progress.progress(`Checking health of ${urls.length} sitemap URLs (est. ${estimateLabel})...`, 'sitemap-health-check');
