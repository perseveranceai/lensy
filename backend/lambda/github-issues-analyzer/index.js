...
async function putDomainCache(domain, cache) {
    if (!ANALYSIS_BUCKET)
        return;
...
// ─── HTTP Helper ─────────────────────────────────────────────────────────────
function makeRequest(url, headers = {}, method = 'GET', body) {
...
async function fetchRecentIssues(owner, repo) {
    ...
    for (let page = 1; page <= MAX_PAGES; page++) {
        ...
        if (items.length === 0)
            break;
        allIssues.push(...items.map((issue) => ({
            number: issue.number,
            title: issue.title,
            body: issue.body || '',
...
async function fetchRepoDirectory(owner, repo, path) {
    try {
        const url = `https://api.github.com/repos/${owner}/${repo}/contents/${path}`;
        const response = await makeRequest(url);
        const items = JSON.parse(response);
        if (Array.isArray(items)) {
            return items.map((item) => ({ path: item.path, type: item.type }));
        }
        return [];
    }
    catch {
...
async function fetchDocsContext(owner, repo, docsUrl) {
    const docs = [];
...
async function classifyWithLLM(issues, progressPublisher) {
    if (issues.length === 0)
        return new Map();
    const BATCH_SIZE = 50;
    const classifiedIssues = new Map();
...
{"docsRelated": [{"number": <issue_number>, "score": <0-100>, "categories": ["<tag1>", "<tag2>"]}]}`;
        try {
            ...
            (parsed.docsRelated || []).forEach((item) => {
...
function classifyDocsRelatedFallback(issue) {
    ...
    if (issue.labels.some(l => docsLabels.includes(l.name.toLowerCase())))
        return true;
    if (strongSignals.some(s => title.includes(s)))
        return true;
    if (strongSignals.some(s => body.includes(s)))
        return true;
...
async function analyzeIssueAgainstDocs(issue, docs) {
...
async function discoverDocsRepos(owner, currentRepo) {
    try {
        ...
        const candidates = repos
            ...
            return namePatterns.some(re => re.test(name)) ||
                ...
                topics.some((t) => topicPatterns.some(re => re.test(t)));
        })
            .slice(0, 5);
        ...
        for (const r of candidates) {
            ...
            try {
                ...
                const codeFileCount = rootItems.filter(i => i.type === 'file' && codeExtensions.some(ext => i.path.toLowerCase().endsWith(ext))).length;
                ...
                if (hasLlmsTxt) {
                    ...
                    const rootMdFiles = rootItems.filter(i => i.type === 'file' &&
...
async function fetchDocsFromGitHubRepo(docsOwner, docsRepo) {
    const docs = [];
    console.log(`  Fetching docs from GitHub repo: ${docsOwner}/${docsRepo}...`);
    // Fetch root-level MD/MDX files
    try {
        const rootItems = await fetchRepoDirectory(docsOwner, docsRepo, '');
        const mdFiles = rootItems.filter(item => item.type === 'file' && (item.path.endsWith('.md') || item.path.endsWith('.mdx')));
        ...
        const subDirs = rootItems.filter(item => item.type === 'dir' && ['docs', 'content', 'pages', 'src'].includes(item.path.toLowerCase()));
        for (const dir of subDirs) {
            try {
                const dirItems = await fetchRepoDirectory(docsOwner, docsRepo, dir.path);
                const subMdFiles = dirItems.filter(item => item.type === 'file' && (item.path.endsWith('.md') || item.path.endsWith('.mdx')));
...
async function selectRelevantDocsPages(entries, issues) {
...
async function crawlAllPages(entries, publisher) {
    const BATCH_SIZE = 10;
    const results = [];
    ...
    for (let i = 0; i < entries.length; i += BATCH_SIZE) {
        ...
        await publisher?.progress(`Crawling batch ${batchNum}/${totalBatches} (${batch.length} pages)...`, 'url-processing', { processingTime: batchNum });
...
async function summarizeAllPages(crawledPages, publisher) {
    ...
    for (let i = 0; i < crawledPages.length; i += BATCH_SIZE) {
        ...
        await publisher?.progress(`Summarizing batch ${batchNum}/${totalBatches} (${batch.length} pages)...`, 'structure-detection', { processingTime: batchNum });
...
async function crawlDocsWebsite(siteUrl, issues, publisher) {
    ...
    if (llmsTxtEntries.length === 0) {
        ...
        try {
            ...
            if (docsUrls.length === 0) {
                docsUrls = allUrls.filter(u => !u.match(/\.(jpg|png|gif|svg|css|js|pdf|zip|ico)$/i) &&
    ...
    if (issues && issues.length > 0 && cachedPages.length > 10) {
        ...
        const entriesWithSummaries = cachedPages.map(p => ({
            title: p.title,
            url: p.url,
...
const handler = async (event) => {
    ...
    const publisher = sessionId ? new progress_publisher_1.ProgressPublisher(sessionId) : undefined;
    const corsHeaders = {
    ...
    try {
        ...
        if (mode === 'analyze' || issues) {
            ...
            for (let idx = 0; idx < issuesToProcess.length; idx++) {
                ...
                await publisher?.progress(`Analyzing issue ${idx + 1}/${issuesToProcess.length}: #${issue.number} ${issue.title.substring(0, 60)}`, 'dimension-analysis', { dimension: `issue-${issue.number}` });
        ...
        if (mode === 'create-pr') {
            ...
            await makeRequest(`https://api.github.com/repos/${owner}/${repo}/git/refs`, authHeaders, 'POST', JSON.stringify({ ref: `refs/heads/${branchName}`, sha: baseSha }));
            ...
            let fileSha = '';
            try {
                const fileData = JSON.parse(await makeRequest(`https://api.github.com/repos/${owner}/${repo}/contents/${cleanPath}?ref=${defaultBranch}`, authHeaders));
            ...
            await makeRequest(`https://api.github.com/repos/${owner}/${repo}/contents/${cleanPath}`, authHeaders, 'PUT', JSON.stringify(putBody));
            ...
            const prResponse = JSON.parse(await makeRequest(`https://api.github.com/repos/${owner}/${repo}/pulls`, authHeaders, 'POST', JSON.stringify({
                title: prTitle,
                body: prBody,
                head: branchName,
                base: defaultBranch
