...
function LensyApp() {
    ...
    /** Export AI Readiness report as neutral-branded Markdown */
    const exportAIReadinessMarkdown = (report: any) => {
        const brandNeutral = process.env.REACT_APP_BRAND_NEUTRAL === 'true';
        const generator = brandNeutral ? 'AI Docs Readiness Checker' : 'Lensy';
        const analysisDate = new Date().toLocaleDateString('en-US', {
            year: 'numeric', month: 'long', day: 'numeric'
        });
        const targetUrl = report.scope?.url || url || '(unknown)';

        let md = `# AI Readiness Report\n\n`;
        md += `**URL:** ${targetUrl}\n`;
        md += `**Analyzed:** ${analysisDate}\n`;
        md += `**Overall Score:** ${report.overallScore}/100 (${report.letterGrade})\n`;
        md += `**Bot Access:** ${report.botAccessState}\n\n`;

        md += `---\n\n`;
        md += `## Score Breakdown\n\n`;
        md += `| Dimension | Score |\n`;
        md += `|---|---|\n`;
        md += `| Discoverability | ${report.scoreBreakdown.discoverability} / 40 |\n`;
        md += `| LLM Consumability | ${report.scoreBreakdown.consumability} / 40 |\n`;
        md += `| Structured Data | ${report.scoreBreakdown.structuredData} / 20 |\n`;
        md += `\n`;

        if (report.aiDiscoverability) {
            md += `## AI Search Citations\n\n`;
            const cited = report.aiDiscoverability.citedCount ?? 0;
            const total = report.aiDiscoverability.totalQueries ?? 0;
            md += `Cited in **${cited} of ${total}** synthetic AI search queries.\n\n`;
            if (report.aiDiscoverability.citationRate) {
                md += `Citation rate: ${report.aiDiscoverability.citationRate}\n\n`;
            }
        }

        if (report.contextualSuggestions && report.contextualSuggestions.length > 0) {
            md += `## Contextual Suggestions\n\n`;
            report.contextualSuggestions.forEach((s: string) => {
                md += `- ${s}\n`;
            });
            md += `\n`;
        }
        ...
        if (report.recommendations && report.recommendations.length > 0) {
            md += `## Recommendations\n\n`;
            const high = report.recommendations.filter((r: any) => r.priority === 'high');
            const medium = report.recommendations.filter((r: any) => r.priority === 'medium');
            const low = report.recommendations.filter((r: any) => r.priority === 'low');

            const writeRec = (rec: any, idx: number) => {
                md += `### ${idx + 1}. ${rec.issue}\n\n`;
            ...
            if (high.length) {
                md += `### High Priority\n\n`;
                high.forEach((r: any, i: number) => writeRec(r, i));
            }
            if (medium.length) {
                md += `### Medium Priority\n\n`;
                medium.forEach((r: any, i: number) => writeRec(r, i));
            }
            if (low.length) {
                md += `### Low Priority\n\n`;
                low.forEach((r: any, i: number) => writeRec(r, i));
            }
        }

        md += `---\n\n`;
        md += `*Report generated by ${generator}. Analysis time: ${(report.analysisTime / 1000).toFixed(1)}s.*\n`;
        ...
        try { safeDomain = new URL(targetUrl).hostname.replace(/[^a-zA-Z0-9]/g, '-'); } catch { }
        a.download = `ai-readiness-${safeDomain}-${new Date().toISOString().split('T')[0]}.md`;
        document.body.appendChild(a);
        a.click();
        setTimeout(() => {
            document.body.removeChild(a);
                                    ...
                                    {analysisState.report && (
                                        <Box sx={{ mt: 2, display: 'flex', justifyContent: 'flex-end' }}>
                                            <Button
                                                variant="outlined"
                                                size="small"
                                                onClick={() => exportAIReadinessMarkdown(analysisState.report)}
